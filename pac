var FindProxyForURL = function(init, profiles) {
    return function(url, host) {
        "use strict";
        var result = init, scheme = url.substr(0, url.indexOf(":"));
        do {
            result = profiles[result];
            if (typeof result === "function") result = result(url, host, scheme);
        } while (typeof result !== "string" || result.charCodeAt(0) === 43);
        return result;
    };
}("+auto", {
    "+auto": function(url, host, scheme) {
        "use strict";
        if (/(?:^|\.)page\.link$/.test(host)) return "+proxy";
        if (/(?:^|\.)gstatic\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)shuziyimin\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)windcave\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)one\.nz$/.test(host)) return "+proxy";
        if (/(?:^|\.)vodafone\.co\.nz$/.test(host)) return "+proxy";
        if (/(?:^|\.)scene7\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)quantummetric\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)t-mobile\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)wtxx\.buzz$/.test(host)) return "+proxy";
        if (/(?:^|\.)widgetcore\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)apkpure\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)uptodown\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)typecho\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)godaddy\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)onetrust\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)oid-info\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)cookielaw\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)adobedtm\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)salesforceliveagent\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)digicert\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)unpkg\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)bootstrapstudio\.io$/.test(host)) return "+proxy";
        if (/(?:^|\.)onecompiler\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)filestackapi\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)codepen\.io$/.test(host)) return "+proxy";
        if (/(?:^|\.)oidref\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)azureedge\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)microsoft\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)slack\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)nist\.gov$/.test(host)) return "+proxy";
        if (/(?:^|\.)zdassets\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)sandboxie-plus\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)displaycal\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)blogspot\.se$/.test(host)) return "+proxy";
        if (/(?:^|\.)mariadb\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)live\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)gitlab\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ycombinator\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)git-scm\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)fosshub\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)qbittorrent\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)bitcomet\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)slproweb\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)medium\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)theb\.ai$/.test(host)) return "+proxy";
        if (/(?:^|\.)bing\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)raw\.githubusercontent\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)python\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)rutracker\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)redditstatic\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)objects\.githubusercontent\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googlevideo\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ggpht\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ytimg\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ami\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)stripe\.network$/.test(host)) return "+proxy";
        if (/(?:^|\.)stripe\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)fontawesome\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)github\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)termius\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googleapis\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)android\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)moapp\.me$/.test(host)) return "+proxy";
        if (/(?:^|\.)stackexchange\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)transparency\.dev$/.test(host)) return "+proxy";
        if (/(?:^|\.)commondatastorage\.googleapis\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)announcekit\.app$/.test(host)) return "+proxy";
        if (/(?:^|\.)narkive\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)ajax\.googleapis\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)google\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)addthis\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)omappapi\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)techtarget\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)venafi\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)driftt\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ubembed\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)aiuys\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)telegram\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)who\.is$/.test(host)) return "+proxy";
        if (/(?:^|\.)mega\.co\.nz$/.test(host)) return "+proxy";
        if (/(?:^|\.)cloudcone\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)fastpanel\.direct$/.test(host)) return "+proxy";
        if (/(?:^|\.)wikimedia\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)libera\.chat$/.test(host)) return "+proxy";
        if (/(?:^|\.)codeproject\.global\.ssl\.fastly\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)codeproject\.freetls\.fastly\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)oss\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)obj-sys\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)jquery\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)pkisolutions\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googlegroups\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ritter\.vg$/.test(host)) return "+proxy";
        if (/(?:^|\.)letsencrypt\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)etsi\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)sysadmins\.lv$/.test(host)) return "+proxy";
        if (/(?:^|\.)gravatar\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)hinet\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)crt\.sh$/.test(host)) return "+proxy";
        if (/(?:^|\.)sidstamm\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)bp\.blogspot.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)microsofttranslator\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)blogblog\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)blogger\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)honestachmed\.dyndns\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)chopsdash\.appspot\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)chromium\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)arm\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)openwrt\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)renwox\.cn$/.test(host)) return "+proxy";
        if (/(?:^|\.)reddit\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)nvda\.ws$/.test(host)) return "+proxy";
        if (/(?:^|\.)versus\.io$/.test(host)) return "+proxy";
        if (/(?:^|\.)telegram\.me$/.test(host)) return "+proxy";
        if (/(?:^|\.)t\.me$/.test(host)) return "+proxy";
        if (/(?:^|\.)doubleclick\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)googleusercontent\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)google-analytics\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googlesource\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)imgur\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)twimg\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)twitter\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)youtube\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)steamcommunity\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)xiaomi\.eu$/.test(host)) return "+proxy";
        if (/(?:^|\.)wikipedia\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)mega\.nz$/.test(host)) return "+proxy";
        if (/(?:^|\.)gmail\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ycombinator\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)git-scm\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)fosshub\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)bitcomet\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)poperblocker\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)manizx\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)anti-ad\.win$/.test(host)) return "+proxy";
        if (/(?:^|\.)gfw\.report$/.test(host)) return "+proxy";
        if (/(?:^|\.)doubleclick\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)google-analytics\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googletagmanager\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)regery\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)regery\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)mxpnl\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)easydmarc\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)hs-scripts\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)private-user-images\.githubusercontent\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)camo\.githubusercontent\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)githubassets\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)user-images\.githubusercontent\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)spartanhost\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)spamhaus\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)ssllabs\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)openstreetmap\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)openmaptiles\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)gstatic\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)vercel\.app$/.test(host)) return "+proxy";
        if (/(?:^|\.)spartanhost\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)livechatinc\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)cdn-telegram\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)cloudflare\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)pkgs\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)gitlab\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)v2ray\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)outbrain\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)wp\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)racknerd\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)speedtest\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)element\.io$/.test(host)) return "+proxy";
        if (/(?:^|\.)matrix\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)singpost\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)steamstatic\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)youtube-nocookie\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)openai\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)jetbrains\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)fauxid\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)liberty\.edu$/.test(host)) return "+proxy";
        if (/(?:^|\.)phoronix\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)rubiconproject\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)fbcdn\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)sharethrough\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)facebook\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)criteo\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)cloudfront\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)d1wnclalxop6x4\.cloudfront\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)facebook\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)adobedtm\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)reddit\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)redditmedia\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)aka\.ms$/.test(host)) return "+proxy";
        if (/(?:^|\.)steampowered\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)shuzijumin\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)softpedia\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)stackadapt\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)id5-sync\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)openx\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)ipredictive\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)e-volution\.ai$/.test(host)) return "+proxy";
        if (/(?:^|\.)ezodn\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)andbeyond\.media$/.test(host)) return "+proxy";
        if (/(?:^|\.)outbrainimg\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)apkmirror\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)google\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ytimg\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)youtube\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)avatars\.githubusercontent\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)objects\.githubusercontent\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)daumcdn\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)g\.co$/.test(host)) return "+proxy";
        if (/(?:^|\.)oaistatic\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)tawk\.to$/.test(host)) return "+proxy";
        if (/(?:^|\.)crisp\.chat$/.test(host)) return "+proxy";
        if (/(?:^|\.)cloudcone\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)google\.cn$/.test(host)) return "+proxy";
        if (/(?:^|\.)geekuninstaller\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)steamdb\.info$/.test(host)) return "+proxy";
        if (/(?:^|\.)arc\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)winos\.vip$/.test(host)) return "+proxy";
        if (/(?:^|\.)microsoft\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)internetdownloadmanager\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)digitalocean\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)css-tricks\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)v2ex\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googles\.plus$/.test(host)) return "+proxy";
        if (/(?:^|\.)jsdelivr\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)golang\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)slproweb\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)arkoselabs\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)octocaptcha\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)github\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)chromium\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)certificate-transparency\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)intercom\.io$/.test(host)) return "+proxy";
        if (/^1\.1\.1\.1$/.test(host)) return "+proxy";
        if (/(?:^|\.)dropbox\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)altervista\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)mediafire\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)webvisor\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)youtu\.be$/.test(host)) return "+proxy";
        if (/(?:^|\.)brave\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)archive\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)wireshark\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)csn\.edu$/.test(host)) return "+proxy";
        if (/(?:^|\.)portalas\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)thinkbabynames\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)apidock\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)opensource\.google$/.test(host)) return "+proxy";
        if (/(?:^|\.)temu\.com$/.test(host)) return "+proxy";
        if (/goo\.gle$/.test(host)) return "+proxy";
        if (/(?:^|\.)dfox\.it$/.test(host)) return "+proxy";
        if (/(?:^|\.)mytheme\.cn$/.test(host)) return "+proxy";
        if (/(?:^|\.)huaqi\.host$/.test(host)) return "+proxy";
        if (/(?:^|\.)bootstrapcdn\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)logrocket\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)fastcdn\.co$/.test(host)) return "+proxy";
        if (/(?:^|\.)fyne\.io$/.test(host)) return "+proxy";
        if (/(?:^|\.)whoer\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)evondt\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)tdesktop\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)adminun\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)coderfan\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)tiqcdn\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)appspot\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)shaka-player-demo\.appspot\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)blogspot\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)t\.co$/.test(host)) return "+proxy";
        if (/(?:^|\.)cloud-dot-devsite-v2-prod\.appspot\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)csp\.withgoogle\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)usps\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)stripe\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)workers\.dev$/.test(host)) return "+proxy";
        if (/(?:^|\.)nmap\.online$/.test(host)) return "+proxy";
        if (/(?:^|\.)dns\.google$/.test(host)) return "+proxy";
        if (/(?:^|\.)1kbtool\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ylibrary\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)ktxr\.rs$/.test(host)) return "+proxy";
        if (/(?:^|\.)momot\.rs$/.test(host)) return "+proxy";
        if (/(?:^|\.)sentry-cdn\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)zhelper\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)theb\.ai$/.test(host)) return "+proxy";
        if (/(?:^|\.)digitaloceanspaces\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)rutracker\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)iyzico\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)netspotapp\.com$/.test(host)) return "+proxy";
        if (/^95\.161\.217\.69$/.test(host)) return "+proxy";
        if (/(?:^|\.)qbittorrent\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)telegra\.ph$/.test(host)) return "+proxy";
        if (/(?:^|\.)quora\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)pinimg\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)redditstatic\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)fontawesome\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)powrcdn\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ggpht\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googlevideo\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)letsvpn\.world$/.test(host)) return "+proxy";
        if (/(?:^|\.)googleblog\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)raw\.githubusercontent\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)gerritcodereview\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)giffgaff\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)tiktok\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)snapchat\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)akr-developers\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)disqus\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)medium\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)boileryao\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)lncld\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)jitpack\.io$/.test(host)) return "+proxy";
        if (/(?:^|\.)ami\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)stripe\.network$/.test(host)) return "+proxy";
        if (/(?:^|\.)termius\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googleapis\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)android\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)moapp\.me$/.test(host)) return "+proxy";
        if (/(?:^|\.)stackexchange\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)transparency\.dev$/.test(host)) return "+proxy";
        if (/(?:^|\.)commondatastorage\.googleapis\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)announcekit\.app$/.test(host)) return "+proxy";
        if (/(?:^|\.)narkive\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)ajax\.googleapis\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)addthis\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)omappapi\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)techtarget\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)venafi\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)driftt\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ubembed\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)aiuys\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)telegram\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)who\.is$/.test(host)) return "+proxy";
        if (/(?:^|\.)mega\.co\.nz$/.test(host)) return "+proxy";
        if (/(?:^|\.)fastpanel\.direct$/.test(host)) return "+proxy";
        if (/(?:^|\.)wikimedia\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)libera\.chat$/.test(host)) return "+proxy";
        if (/(?:^|\.)codeproject\.global\.ssl\.fastly\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)codeproject\.freetls\.fastly\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)oss\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)obj-sys\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)jquery\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)pkisolutions\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googlegroups\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)ritter\.vg$/.test(host)) return "+proxy";
        if (/(?:^|\.)letsencrypt\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)etsi\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)sysadmins\.lv$/.test(host)) return "+proxy";
        if (/(?:^|\.)gravatar\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)hinet\.net$/.test(host)) return "+proxy";
        if (/(?:^|\.)crt\.sh$/.test(host)) return "+proxy";
        if (/(?:^|\.)sidstamm\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)bp\.blogspot\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)microsofttranslator\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)blogblog\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)blogger\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)honestachmed\.dyndns\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)oid-info\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)chopsdash\.appspot\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)arm\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)openwrt\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)renwox\.cn$/.test(host)) return "+proxy";
        if (/(?:^|\.)nvda\.ws$/.test(host)) return "+proxy";
        if (/(?:^|\.)versus\.io$/.test(host)) return "+proxy";
        if (/(?:^|\.)telegram\.me$/.test(host)) return "+proxy";
        if (/(?:^|\.)t\.me$/.test(host)) return "+proxy";
        if (/(?:^|\.)googleusercontent\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googlesource\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)imgur\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)twimg\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)twitter\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)steamcommunity\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)xiaomi\.eu$/.test(host)) return "+proxy";
        if (/(?:^|\.)wikipedia\.org$/.test(host)) return "+proxy";
        if (/(?:^|\.)mega\.nz$/.test(host)) return "+proxy";
        if (/(?:^|\.)gmail\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googleapis\.com$/.test(host)) return "+proxy";
        if (/^translate\.googleapis\.com$/.test(host)) return "+proxy";
        if (/(?:^|\.)googleapis\.cn$/.test(host)) return "+proxy";
        if (/(?:^|\.)google\.hk$/.test(host)) return "+proxy";
        if (/(?:^|\.).*\.googleapis\.com$/.test(host)) return "+proxy";
        return "DIRECT";
    },
    "+proxy": function(url, host, scheme) {
        "use strict";
        if (/^127\.0\.0\.1$/.test(host) || /^::1$/.test(host) || /^localhost$/.test(host) || /^192\.168\.1\.1$/.test(host) || /^192\.168\.4\.1$/.test(host)) return "DIRECT";
        return "PROXY 127.0.0.1:1080";
    }
});